...
import type { PopulateDocumentCountsInput } from '@documenso/lib/server-only/document/populate-document-counts';
import { populateDocumentCounts } from '@documenso/lib/server-only/document/populate-document-counts';
...
export const DocumentsPageView = async ({ searchParams = {}, team }: DocumentsPageViewProps) => {
  ...
  const getStatOptions: PopulateDocumentCountsInput = {
    user,
    period,
  };
  ...
  const stats = await populateDocumentCounts(getStatOptions);
